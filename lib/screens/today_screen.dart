...
class _TodayScreenState extends State<TodayScreen> with WidgetsBindingObserver {
  ...
  Future<bool> _discard({bool changingDate = true}) async {
    ...
    return await showDialog<bool>(
          ...
          builder: (context) => AlertDialog(
            ...
            content: Text(
              changingDate ? '입력을 버리고 날짜를 변경할까요?' : '저장하지 않고 입력 화면을 닫을까요?',
            ),
            actions: [
              TextButton(
              ...
              TextButton(
                ...
                child: Text(changingDate ? '날짜 변경' : '닫기'),
              ),
            ],
          ),
        ) ??
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      onPopInvokedWithResult: (didPop, result) async {
        ...
        if (await _discard(changingDate: false) && mounted) {
          setState(() => _savedInput = _input);
